...
from model.net.vqseg_v2 import VQSeg
...
class ModelModule(LightningModule):
    ...
    def configure_losses(self):
        self.loss_fn = {
            ...
            # 'quant_vector_loss': 0,
            'disc_loss': MSELoss(),
            'point_loss': CrossEntropyLoss(ignore_index=255),
        }
    ...
    def calculate_loss(self, out, batch, batch_idx, mode):
        ...
        for loss_name, f in self.loss_fn.items():
            if loss_name == "segment_loss":
                ...
                loss = quant_loss
            elif loss_name == 'quant_vector_loss':
                codebook = self.vqseg.codebook.embedding_key.weight
                loss = (codebook @ codebook.t()).mean() ** 2
            elif loss_name == 'disc_loss':
                disc_label = torch.nn.functional.interpolate(annotations.float().unsqueeze(dim=1), size=quant_disc.shape[2:], mode='nearest')
                loss = f(quant_disc, disc_label)
