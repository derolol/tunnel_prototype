...
class Detail(nn.Module):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.fea1 = ConvModule(c1=3, c2=16, k=3, s=2, p=1)
        self.fea2 = ConvModule(c1=16, c2=16, k=3, s=1, p=1)
        self.fea3 = ConvModule(c1=16, c2=16, k=3, s=1, p=1)

    def forward(self, x):
        return self.fea3(self.fea2(self.fea1(x)))
...
class SegFormer(nn.Module):
    def __init__(self,
        ...
        self.detail = Detail()
        self.down1 = ConvModule(c1=int(embed_dims[0] / 2), c2=embed_dims[0], k=3, s=2, p=1)
        self.down2 = ConvModule(c1=embed_dims[0], c2=embed_dims[0], k=3, s=1, p=1)
        self.down3 = ConvModule(c1=embed_dims[0], c2=embed_dims[0], k=3, s=1, p=1)
        
        self.concat = ConvModule(c1=embed_dims[0] * 2, c2=embed_dims[0], k=1, s=1, p=0)
    ...
    def forward(self, inputs):
        ...
        detail = self.detail(inputs)
        down = self.down3(self.down2(self.down1(detail)))
        new_x0 = self.concat(torch.concat([x[0], down], dim=1))
        x[0] = new_x0
...
if __name__ == '__main__':
    ...
    x, output_tensor = model(input_tensor)
