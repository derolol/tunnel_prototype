...
class Block(nn.Module):
    ...
    def forward(self, x, query, H, W):
        x = x + self.drop_path(self.attn(self.norm1(x), self.query_norm1(query), H, W))
...
class MixVisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.query_patch_embed1 = OverlapPatchEmbed(in_channels=in_channels, 
                                              patch_size=7, stride=4, 
                                              embed_dim=embed_dims[0])
        #-----------------------------------------------#
        ...
        self.query_block1 = nn.ModuleList(
            [
                Block(
                    dim=embed_dims[0], num_heads=num_heads[0], mlp_ratio=mlp_ratios[0], qkv_bias=qkv_bias, qk_scale=qk_scale,
                    drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer, sr_ratio=sr_ratios[0]
        ...
        self.query_patch_embed2 = OverlapPatchEmbed(in_channels=embed_dims[0],
                                              patch_size=3, stride=2,
                                              embed_dim=embed_dims[1])
        #-----------------------------------------------#
        #   利用transformer模块进行特征提取
        ...
        self.query_block2 = nn.ModuleList(
            [
                Block(
                    dim=embed_dims[1],
                    num_heads=num_heads[1], mlp_ratio=mlp_ratios[1],
        ...
        self.query_patch_embed3 = OverlapPatchEmbed(in_channels=embed_dims[1],
                                              patch_size=3, stride=2,
                                              embed_dim=embed_dims[2])
        #-----------------------------------------------#
        #   利用transformer模块进行特征提取
        ...
        self.query_block3 = nn.ModuleList(
            [
                Block(
                    dim=embed_dims[2],
                    num_heads=num_heads[2], mlp_ratio=mlp_ratios[2], 
        ...
        self.query_patch_embed4 = OverlapPatchEmbed(in_channels=embed_dims[2],
                                              patch_size=3, stride=2,
                                              embed_dim=embed_dims[3])
        #-----------------------------------------------#
        #   利用transformer模块进行特征提取
        ...
        self.query_block4 = nn.ModuleList(
            [
                Block(
                    dim=embed_dims[3],
                    num_heads=num_heads[3], mlp_ratio=mlp_ratios[3],
    ...
    def forward(self, x):
        ...
        #   block1
        #----------------------------------#
        patch1, H, W = self.patch_embed1.forward(x)
        query1, H, W = self.query_patch_embed1.forward(x)
        for i, blk in enumerate(self.block1):
            query1 = self.query_block1[i](query1, query1, H, W)
            patch1 = blk.forward(patch1, query1, H, W)
        x = self.norm1(patch1)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        query1 = query1.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        outs.append(x)
        ...
        #   block2
        #----------------------------------#
        patch2, H, W = self.patch_embed2.forward(x)
        query2, H, W = self.query_patch_embed2.forward(query1)
        for i, blk in enumerate(self.block2):
            query2 = self.query_block2[i](query2, query2, H, W)
            patch2 = blk.forward(patch2, query2, H, W)
        x = self.norm2(patch2)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        query2 = query2.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        outs.append(x)
        ...
        #   block3
        #----------------------------------#
        patch3, H, W = self.patch_embed3.forward(x)
        query3, H, W = self.query_patch_embed3.forward(query2)
        for i, blk in enumerate(self.block3):
            query3 = self.query_block3[i](query3, query3, H, W)
            patch3 = blk.forward(patch3, query3, H, W)
        x = self.norm3(patch3)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        query3 = query3.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        outs.append(x)
        querys.append(query3)

        #----------------------------------#
        #   block4
        #----------------------------------#
        patch4, H, W = self.patch_embed4.forward(x)
        query4, H, W = self.query_patch_embed4.forward(query3)
        for i, blk in enumerate(self.block4):
            query4 = self.query_block4[i](query4, query4, H, W)
            patch4 = blk.forward(patch4, query4, H, W)
        x = self.norm4(patch4)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        query4 = query4.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        outs.append(x)
        querys.append(query4)
...
class CrossSegFormer(nn.Module):
    def __init__(self,
        ...
        if pretrained_path:
            ...
            for key, item in model_state_dict.items():
                # copy
                if key.find('query') != -1:
                    src_key = key.replace('query_', '')
                    while src_key.find('query') != -1:
                        src_key = src_key.replace('query_', '')
                    new_state_dict[key] = state_dict[src_key]
                # keep
                else:
                    new_state_dict[key] = item
            self.backbone.load_state_dict(state_dict=new_state_dict, strict=True)
    ...
    def forward(self, inputs):
        ...
        x, querys = self.backbone.forward(inputs)
...
if __name__ == '__main__':
    ...
    output_tensor, querys = model(input_tensor)
