...
class SegEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(len(embed_dims)):
            ...
            pool = nn.Conv2d(in_channels=output_channel, out_channels=output_channel,
                             kernel_size=3, stride=2, padding=1)
...
class Codebook(nn.Module):
    ...
    def __init__(self, num_vectors, latent_dim, beta):
        ...
        # orthogonal_tensor = torch.randn(self.latent_dim, self.latent_dim)
        # torch.nn.init.orthogonal_(orthogonal_tensor, gain=1)
        # self.embedding_value = nn.Parameter(orthogonal_tensor, requires_grad=True)
    ...
    def forward(self, x):
        ...
        # self.embedding_key.weight.data = nn.functional.softmax(self.embedding_key.weight, dim=1)
        ...
        # (b x h x w, latent_dim)
        # x_q = (x_q @ self.embedding_value)
        # x_q = (x_q @ self.embedding_value).view(x.shape)
        # x_q = nn.functional.softmax(x_q, dim=1)
        # (b, h, w, latent_dim)
...
class SegDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        for dim_index in range(len(embed_dims) - 1):

            in_dim = embed_dims[dim_index]
            linear_dim = embed_dims[dim_index + 1]
            # print("in:", in_dim, "out", linear_dim)

            linear = nn.Linear(in_features=in_dim, out_features=linear_dim)
            self.linear_list.append(linear)

            conv = nn.Sequential(
                nn.Conv2d(in_channels=linear_dim * 2,
                          out_channels=linear_dim,
                           kernel_size=3,
                           stride=1,
                           padding=1),
                nn.BatchNorm2d(num_features=linear_dim),
                nn.ReLU(),
                nn.Conv2d(in_channels=linear_dim,
                          out_channels=linear_dim,
                           kernel_size=3,
                           stride=1,
                           padding=1),
        ...
        self.seg = nn.Conv2d(in_channels=embed_dims[-1], out_channels=3,
                             kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x, image):
        ...
        self.output_size = [s * 2 for s in self.output_size]
        # print("decoder output", self.output_size)
        ...
        for index in range(1, len(x)):
            ...
            linear = self.linear_list[index - 1]
            B, C, H, W = fea1.shape
            fea1 = rearrange(fea1, 'b c h w -> b (h w) c')
            fea1 = linear(fea1)
            fea1 = rearrange(fea1, 'b (h w) c -> b c h w', h=H, w=W)

            fea1 = F.interpolate(fea1, size=fea2.shape[2:], mode='bilinear')

            conv = self.conv_list[index - 1]
            fea1 = conv(torch.concat([fea1, fea2], dim=1))
...
class VQSeg(nn.Module):
    ...
    def forward(self, x):
        ...
        return x, x_indices, quant_loss, x_quant_grad, points
