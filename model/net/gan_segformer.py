...
class EdgeTransformer(nn.Module):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.patch_embed1 = OverlapPatchEmbed(in_channels=32, 
                                              patch_size=1,
                                              stride=1,
                                              embed_dim=32)
        #-----------------------------------------------#
        ...
        self.block1 = nn.ModuleList(
            [
                Block(
                    dim=32, num_heads=1, mlp_ratio=4, qkv_bias=False, qk_scale=None,
                    drop=0., attn_drop=0., drop_path=0., norm_layer=nn.LayerNorm, sr_ratio=8
                )
                for i in range(4)
            ]
        )
...
class MixVisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        # self.conv_dim = ConvModule(c1=128, c2=32, k=1, s=1, p=0)
    ...
    def forward(self, x):
        ...
        for i, blk in enumerate(self.block1):
            x = blk.forward(x, H, W)
        x = self.norm1(x)
        generate_x = x + generate_x # res block
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        generate_x = generate_x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        outs.append(x)
...
class SegFormerHead(nn.Module):
    ...
    def __init__(self, num_classes=20, in_channels=[32, 64, 160, 256], head_embed_dim=768, dropout_ratio=0.1):
        ...
        self.linear_gen = MLP(input_dim=c1_in_channels, embed_dim=head_embed_dim)

        self.edge_attention = Block(dim=head_embed_dim,
                                    num_heads=8,
                                    mlp_ratio=4,
                                    qkv_bias=False,
                                    qk_scale=None,
                                    ...
                                    norm_layer=nn.LayerNorm,
                                    sr_ratio=8)
        self.norm = nn.LayerNorm(normalized_shape=head_embed_dim)

        self.edge_skff1 = SKFF(in_channels=head_embed_dim, height=2)
        self.edge_skff2 = SKFF(in_channels=head_embed_dim, height=2)
        self.edge_skff3 = SKFF(in_channels=head_embed_dim, height=2)
        self.edge_skff4 = SKFF(in_channels=head_embed_dim, height=2)

        self.edge_fuse1 = ConvModule(c1=(head_embed_dim * 2), c2=head_embed_dim, k=1)
        self.edge_fuse2 = ConvModule(c1=(head_embed_dim * 2), c2=head_embed_dim, k=1)
        self.edge_fuse3 = ConvModule(c1=(head_embed_dim * 2), c2=head_embed_dim, k=1)
        self.edge_fuse4 = ConvModule(c1=(head_embed_dim * 2), c2=head_embed_dim, k=1)
    ...
    def forward(self, inputs, fuse_gen):
        c1, c2, c3, c4 = inputs
        ...
        _fuse_gen = self.linear_gen(fuse_gen)

        _fuse_gen = self.edge_attention.forward_cross(x=_c1, query=_fuse_gen, H=fuse_gen.shape[2], W=fuse_gen.shape[3])
        _fuse_gen = self.norm(_fuse_gen)
        
        _c1 = _c1.permute(0,2,1).reshape(n, -1, c1.shape[2], c1.shape[3])
        _fuse_gen = _fuse_gen.permute(0,2,1).reshape(n, -1, fuse_gen.shape[2], fuse_gen.shape[3])
        
        _fuse_gen = self.edge_skff1([_fuse_gen, _c1])
        _fuse_gen = self.edge_skff2([_fuse_gen, _c2])
        _fuse_gen = self.edge_skff3([_fuse_gen, _c3])
        _fuse_gen = self.edge_skff4([_fuse_gen, _c4])

        _c1 = self.edge_fuse1(torch.cat([_fuse_gen, _c1], dim=1))
        _c2 = self.edge_fuse2(torch.cat([_fuse_gen, _c2], dim=1))
        _c3 = self.edge_fuse3(torch.cat([_fuse_gen, _c3], dim=1))
        _c4 = self.edge_fuse4(torch.cat([_fuse_gen, _c4], dim=1))
...
class SegFormer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        x, fuse_gen = self.backbone.forward(inputs)
        out = self.decode_head.forward(x, fuse_gen)
        
        fuse_gen = F.interpolate(fuse_gen, size=(H, W), mode='bilinear', align_corners=True)
        out = F.interpolate(out, size=(H, W), mode='bilinear', align_corners=True)
        return x, fuse_gen, out
...
if __name__ == '__main__':
    ...
    _, fuse_gen, output_tensor = model(input_tensor)
    print(fuse_gen.shape, output_tensor.shape)