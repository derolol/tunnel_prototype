...
class EdgeDetect(nn.Module):
    ...
    def forward(self, image):
        ...
        edge_x = F.conv2d(image, self.weight_x).abs()
        edge_y = F.conv2d(image, self.weight_y).abs()
...
class SegDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.aug = ConvModule(in_channels=linear_dim,
    ...
    def forward(self, x):
        ...
        for index in range(len(x) - 1, -1, -1):
            ...
            if index > 0:
                ...
                x_up = F.interpolate(fea,
                                     size=x[index - 1].shape[2 : ],
                                     mode='bilinear')
                x_up = torch.concat([x_up, x[index - 1]], dim=1)
                x[index - 1] = concat(x_up)
                x[index - 1] = x[index - 1] + concat(x_up)
...
class AVQSeg(nn.Module):
    ...
    def __init__(self,
        ...
        if is_seg_refine:
            self.refine_conv = nn.Sequential(
                ConvModule(in_channels=embed_dims[-1],
    ...
    def forward(self, x):
        ...
        if self.is_feature_quant:
            ...
            x_feas[self.quant_dim] = self.quant_fuse([x_fea, x_q])

        seg, x_last = self.decoder(x_feas)

        if self.is_seg_refine:
            x_fea_coarse = x_last
            x_fea_coarse = self.refine_conv(x_fea_coarse)
            refine_res = self.point_head(x, x_fea_coarse, seg)
