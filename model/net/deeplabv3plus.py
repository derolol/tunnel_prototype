# reference: https://github.com/yassouali/pytorch-segmentation
...
class ResNet(nn.Module):
    ...
    def __init__(self, in_channels=3, output_stride=16, backbone='resnet101', pretrained=False):
        ...
        if not pretrained or in_channels != 3:
            ...
            # weight = torch.load(CONFIG.MODEL.PRETRAINED_WEIGHT)
...
class DeepLabV3Plus(nn.Module):
    ...
    def __init__(self,
        ...
        self.is_freeze_backbone = is_freeze_backbone
                
        assert ('xception' or 'resnet' in backbone)
        if 'resnet' in backbone:
            self.backbone = ResNet(in_channels=in_channels, output_stride=output_stride)
        ...
        if self.is_freeze_bn: self.freeze_bn()
        if self.is_freeze_backbone: 
...
if __name__ == '__main__':
    ...
    print(f'Params: {params} FLOPs: {flops}') # Params: 59.340M FLOPs: 88.963G