...
model_state = {
    'b0': '/home/weight/segformer/segformer_b0_backbone_weights.pth',
    'b1': os.path.join(os.path.dirname(__file__), 'segformer_b1_backbone_weights.pth'),
    'b2': os.path.join(os.path.dirname(__file__), 'segformer_b2_backbone_weights.pth'),
    'b3': os.path.join(os.path.dirname(__file__), 'segformer_b3_backbone_weights.pth'),
    'b4': os.path.join(os.path.dirname(__file__), 'segformer_b4_backbone_weights.pth'),
    'b5': os.path.join(os.path.dirname(__file__), 'segformer_b5_backbone_weights.pth')}
...
class mit_b0(MixVisionTransformer):
    def __init__(self, pretrained = False):
        ...
        if pretrained:
            ...
            self.load_state_dict(state_dict=torch.load(model_state['b0']), strict=False)
...
class mit_b1(MixVisionTransformer):
    def __init__(self, pretrained = False):
        ...
        if pretrained:
            print("Load backbone weights")
            self.load_state_dict(state_dict=torch.load(model_state['b1']), strict=False)
...
class mit_b2(MixVisionTransformer):
    def __init__(self, pretrained = False):
        ...
        if pretrained:
            print("Load backbone weights")
            self.load_state_dict(state_dict=torch.load(model_state['b2']), strict=False)
...
class mit_b3(MixVisionTransformer):
    def __init__(self, pretrained = False):
        ...
        if pretrained:
            print("Load backbone weights")
            self.load_state_dict(state_dict=torch.load(model_state['b3']), strict=False)
...
class mit_b4(MixVisionTransformer):
    def __init__(self, pretrained = False):
        ...
        if pretrained:
            print("Load backbone weights")
            self.load_state_dict(state_dict=torch.load(model_state['b4']), strict=False)
...
class mit_b5(MixVisionTransformer):
    def __init__(self, pretrained = False):
        ...
        if pretrained:
            print("Load backbone weights")
            self.load_state_dict(state_dict=torch.load(model_state['b5']), strict=False)
...
class PrototypeSegFormer(nn.Module):
    def __init__(self,
                 phi:str='b0',
                 num_classes:int=4,
        ...
        self.iformer = IformerBlock(dim=self.embedding_dim, num_heads=8)
    ...
    def forward(self, inputs):
        ...
        x, fea, proj32, proj4 = self.decode_head.forward(x)

        fea = fea.permute(0, 2, 3, 1)
        x, hx, lx= self.iformer(fea)
        x = x.permute(0, 3, 1, 2)
    ...
    def layer_norm(self, var):
        B, C, H, W = var.shape
        var_layer = var.view(B, C, -1)
        var_min = torch.min(var_layer, dim=-1, keepdim=True)[0].view(B, C, 1, 1)
        var_max = torch.max(var_layer, dim=-1, keepdim=True)[0].view(B, C, 1, 1)
        return (var - var_min) / (var_max - var_min + 1e-6)
...
class IformerBlock(nn.Module):
    ...
    def forward(self, x):
        if self.use_layer_scale:
            ...
            x, hx, lx = self.attn(self.norm1(x))
...
if __name__ == '__main__':
    ...
    model = PrototypeSegFormer()
    x = torch.randn(size=(1, 3, 512, 512))
    flops, params = profile(model, inputs=(x,))
    flops, params = clever_format([flops, params], '%.3f')
    print(f'Params: {params} FLOPs: {flops}') # Params: 4.436M FLOPs: 18.521G