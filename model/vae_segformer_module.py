...
class ModelModule(LightningModule):
    ...
    def configure_optimizers(self):
        ...
        self.frozen(self.model.backbone.reconstruct1)
        opt_net = torch.optim.Adam(filter(lambda p: p.requires_grad, self.model.parameters()), lr=self.learning_rate)
        self.unfrozen(self.model.backbone.reconstruct1)
        opt_vae = torch.optim.Adam(self.model.backbone.reconstruct1.parameters(), lr=vae_lr, weight_decay=0.001)
