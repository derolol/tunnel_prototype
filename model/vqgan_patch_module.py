...
class ModelModule(LightningModule):
    ...
    def forward(self, batch, mode):
        ...
        if mode == "train":
            ...
            local_images = images.view(B, C, H // patch_size, patch_size, W // patch_size, patch_size)
            local_images = local_images.permute(0, 2, 4, 1, 3, 5).contiguous() # B, a, b, C, p, p
            local_images = local_images.view(-1, C, patch_size, patch_size)

            local_decoded_images = decoded_images.view(B, C, H // patch_size, patch_size, W // patch_size, patch_size)
            local_decoded_images = local_decoded_images.permute(0, 2, 4, 1, 3, 5).contiguous() # B, a, b, C, p, p
            local_decoded_images = local_decoded_images.view(-1, C, patch_size, patch_size)

            # local_size = self.args.image_size // 2
            # local_mask = torch.ones(size=(B, C, local_size, local_size)).to(self.device)
            # local_left = random.randint(a=0, b=(W - local_size - 1))
            # local_top = random.randint(a=0, b=(H - local_size - 1))
            # local_mask = F.pad(local_mask,
            #                    pad=[local_left, local_size - local_left, local_top, local_size - local_top],
            #                    value=0)
            # assert local_mask.shape == images.shape

            mask_images = local_images
            mask_decoded_images = local_decoded_images
